...
def load_and_process_diabetes_data(tokenizer, cutoff_len=512):
    all_data = []
    ...
    for item in all_data:
        ...
        full_prompt = f"""Below is an instruction that describes a task, along with input data. Write a response that appropriately completes the request.
...
{item['output']}"""
        ...
        # Create the user prompt without the response for masking
        user_prompt = f"""Below is an instruction that describes a task, along with input data. Write a response that appropriately completes the request.
...
"""
        ...
        # Create labels with -100 for the user prompt part
        input_ids = tokenized_full_prompt["input_ids"][0]
        user_input_ids_len = len(tokenized_user_prompt["input_ids"][0])
        
        labels = input_ids.clone()
        labels[:user_input_ids_len] = IGNORE_INDEX
        
        processed_item = {
            "input_ids": input_ids,
            "attention_mask": tokenized_full_prompt["attention_mask"][0],
            "labels": labels,
        }
...
def train(
    # model/data params
    base_model: str = "llama-3.1-8B-Instruct",
    data_path: str = "data/diabetes/with_info",
    ...
    train_dataset, eval_dataset = load_and_process_diabetes_data(tokenizer, cutoff_len)
